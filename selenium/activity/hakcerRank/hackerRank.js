...
(async function () {
    try {
        ...
        await driver.get(ManageChallengePage);
        await waitForLoader();
        let AllQuestions = await driver.findElements(swd.By.css(".table-body.mlB.text-center a"));
        for (let i = 0; i < 1; i++) {
            AllQuestions = await driver.findElements(swd.By.css(".table-body.mlB.text-center a"));
            // console.log(AllQuestions);
            let URLToSearch = await AllQuestions[i].getAttribute("href");
            // console.log(URLToSearch);
            await driver.get(URLToSearch);
            await waitForLoader();
            let QuesTabs = await driver.findElements(swd.By.css(".admin.contestedit.container.content_wrap header .tabs-cta-wrapper ul li"));
            await QuesTabs[1].click();

            // console.log(QuesTabs);
            await waitForLoader();
            // setTimeout(() => { }, 10000);
            await driver.wait(swd.until.elementIsNotVisible(swd.By.css("#tags_tag")));
            let moderatorsAdd = await driver.findElement(swd.By.css("#moderator"));
            await moderatorsAdd.sendKeys("sagarGupta0123");
            let AddBtnClick = await driver.findElement(swd.By.css(".btn.moderator-save"));
            await AddBtnClick.click();
            let saveChangesBtnClick = await driver.findElement(swd.By.css(".save-challenge.btn.btn-green"));
            await saveChangesBtnClick.click();
            await waitForLoader();
            let QuesTabsAgain = await driver.findElements(swd.By.css(".admin.contestedit.container.content_wrap header .tabs-cta-wrapper ul li"));
            await QuesTabsAgain[2].click();
            let addTestCase=await driver.findElement(swd.By.css(".row.mlB .pull-right .btn.add-testcase.btn-green"));
            await addTestCase.click();
            let tagsToBeAdded=await driver.findElement(swd.By.css("#tag"));
            tagsToBeAdded.sendKeys(questions[i]["Tags"]);
            let testCaseIn=await driver.findElement(swd.By.css(""));
            await editorHandler(".formgroup.horizontal.input-testcase-row.row .CodeMirror.cm-s-default.CodeMirror-wrap div",".formgroup.horizontal.input-testcase-row.row .CodeMirror.cm-s-default.CodeMirror-wrap div textarea","abc");
